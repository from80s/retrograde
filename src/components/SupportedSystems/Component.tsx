...
function SystemCard({ sys, onClick }: { sys: SystemEntry; onClick?: () => void }) {
  ...
  return (
    ...
    >
      {fanartUrl && (
        <SystemCardBackground $bgUrl={fanartUrl} />
      )}
      {fanartUrl && (
        <SystemCardOverlay />
      )}
      ...
      <SystemCardContent>
        ...
        />
        <SystemNameWrapper>
          <SystemName>{sys.name}</SystemName>
          <SystemShortName>{sys.shortName}</SystemShortName>
        </SystemNameWrapper>
        <ExtensionsContainer>
          {sys.extensions.slice(0, 4).map((ext) => (
            <ExtensionBadge key={ext}>{ext}</ExtensionBadge>
          ))}
          {sys.extensions.length > 4 && (
            <ExtensionMore>+{sys.extensions.length - 4}</ExtensionMore>
          )}
        </ExtensionsContainer>
      </SystemCardContent>
    </SystemCardWrapper>
...
export function SupportedSystems() {
  ...
  return (
    ...
    <GlassContainer>
      ...
      <CarouselWrapper>
        <ScrollButton className="left" onClick={() => scrollByAmount(-300)}>
          <LuChevronLeft />
        </ScrollButton>
        ...
        <ScrollButton className="right" onClick={() => scrollByAmount(300)}>
          <LuChevronRight />
        </ScrollButton>
      </CarouselWrapper>
