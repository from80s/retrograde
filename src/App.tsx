    ...
    <div className="h-screen flex flex-col bg-zinc-950">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <motion.aside
          ...
          onMouseLeave={() => sidebarCollapsed && setSidebarHoverExpanded(false)}
        >
          {/* Logo Header */}
          <div className={`border-b border-zinc-800/50 flex flex-col items-center ${isSidebarExpanded ? 'p-5 gap-2' : 'p-2 gap-0.5'}`}>
            <img src={RetroGradeLogo} alt="RetroGrade" className={`${isSidebarExpanded ? 'h-20' : 'h-8'} w-auto`} />
            {isSidebarExpanded && <p className="text-[10px] text-zinc-500 font-mono">v{version}</p>}
          </div>
          ...
          <button
            ...
            className="absolute top-28 -right-3 w-6 h-10 bg-zinc-800 border border-zinc-700/50 rounded-r-lg flex items-center justify-center text-zinc-400 hover:text-zinc-200 hover:bg-zinc-700 transition-colors z-20"
          >
          ...
          <nav className="flex-1 p-2 space-y-1 overflow-y-auto scrollbar-thin">
            {!apiConnected && isSidebarExpanded && (
              <div className="p-3 bg-retro-warning/5 border border-retro-warning/20 rounded-xl flex items-start gap-2">
                ...
                <div>
                  <p className="text-xs text-retro-warning font-medium">APIs desconectadas</p>
                  <p className="text-[10px] text-zinc-500 mt-0.5">Configure as credenciais em Configurações para habilitar a curadoria.</p>
                </div>
              </div>
            )}
            ...
            >
              <FolderOpen className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
              {isSidebarExpanded && <span className="font-medium text-sm">Selecionar Pasta</span>}
            </button>

            {state.folder && isSidebarExpanded && (
              <div className="px-1 pb-1">
                <div className="glass rounded-lg p-2">
                  <p className="text-[10px] text-zinc-500 mb-0.5">Pasta selecionada</p>
                  <p className="text-xs text-zinc-300 truncate font-mono">{state.folder}</p>
                </div>
              </div>
            )}
            ...
            >
              <Search className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
              {isSidebarExpanded && <span className="font-medium text-sm">Simular Curadoria</span>}
            </button>
            ...
            <button
              ...
              title={!isSidebarExpanded ? (state.isRunning ? 'Processando...' : !apiConnected ? 'APIs desconectadas' : 'Iniciar Curadoria') : undefined}
            >
              {state.isRunning ? (
                <Loader2 className={`flex-shrink-0 animate-spin ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
              ) : (
                <BarChart3 className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
              )}
              {isSidebarExpanded && (
            ...
            >
              <Download className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
              {isSidebarExpanded && <span className="font-medium text-sm">Mídias</span>}
            </button>
            ...
            >
              <Scan className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
              {isSidebarExpanded && <span className="font-medium text-sm">Arquivos Órfãos</span>}
            </button>
            ...
            >
              <Archive className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
              {isSidebarExpanded && <span className="font-medium text-sm">Extrator de ROMs</span>}
            </button>

            {/* Secondary Actions */}
            <div className={`pt-2 border-t border-zinc-800/30 space-y-1 ${!isSidebarExpanded ? 'flex flex-col items-center gap-1' : ''}`}>
              <button
              ...
              >
                <Settings className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
                {isSidebarExpanded && <span className="font-medium text-sm">Configurações</span>}
              </button>
              ...
              >
                <History className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
                {isSidebarExpanded && <span className="font-medium text-sm">Histórico</span>}
              </button>
              ...
              >
                <LifeBuoy className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
                {isSidebarExpanded && <span className="font-medium text-sm">Suporte</span>}
              </button>
              ...
              >
                <Info className={`flex-shrink-0 ${isSidebarExpanded ? 'w-4 h-4' : 'w-5 h-5'}`} />
                {isSidebarExpanded && <span className="font-medium text-sm">Sobre</span>}
              </button>
            </div>
          </nav>

          {/* Buy me a coffee - Bottom of sidebar */}
          <div className={`border-t border-zinc-800/30 ${isSidebarExpanded ? 'p-3' : 'p-2 flex justify-center'}`}>
            <a
            ...
            >
              <Coffee className={isSidebarExpanded ? 'w-3.5 h-3.5' : 'w-4 h-4'} />
              {isSidebarExpanded && <span className="font-medium text-xs">Buy me a coffee</span>}
            </a>
          </div>
        </motion.aside>
